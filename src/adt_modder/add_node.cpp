...
private:
    ...
    [[nodiscard]] const char* Help() const noexcept override {
      return "Adds the given node to the adt in the specified path. If the parent node doesn't exist it fails";
    }

    static bool s_initialized;
};

bool AddNodeOp::s_initialized = AdtModder::RegisterOperation(
    "add_node", AddNodeOp::GetInstance());
...
AdtModder::Result AddNodeOp::Run(AdtModder::Adt adt_data,
  ...
  const auto parent_node_offset = adt_path_offset(adt_data.data(), parent_node_name.c_str());
  const auto next_sibling_offset = adt_next_sibling_offset(adt_data.data(), parent_node_offset);

  // New node will be empty, just header + name property (prop_value = child_node_name + '\0')
  const size_t new_node_size = sizeof(adt_node_hdr) + sizeof(adt_property) + child_node_name.length() + 1;
  ...
  memmove(&adt_data[next_sibling_offset + new_node_size], &adt_data[next_sibling_offset], prev_size - next_sibling_offset);
  ...
  auto name_prop = ADT_PROP(adt_data.data(), adt_first_property_offset(adt_data.data(), child_offset));
